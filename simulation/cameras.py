...
import numpy as np
import pybullet as p  # Assuming pybullet is used for quaternion conversions
...
class RealSenseD415:
    ...
    camera_fov_w = 65.0  # horizontal field of view, width of image
    ...
    image_size = (480, 640)
    camera_fov_w = 65.0  # D415 has a narrower FOV than D435
    camera_focal_length = (float(image_size[1]) / 2) / np.tan((np.pi * camera_fov_w / 180) / 2)
    intrinsics = np.array([[camera_focal_length, 0, image_size[1] / 2],
                           [0, camera_focal_length, image_size[0] / 2],
                           [0, 0, 1]])
